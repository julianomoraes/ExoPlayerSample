...
class EventBusFactory private constructor(val owner: LifecycleOwner) {
    ...
    companion object {
        ...
        fun get(lifecycleOwner: LifecycleOwner): EventBusFactory {
            // Todo for a fragment we should use viewLifecycleOwner() but it may crash if called before onViewCreated()
            return with(lifecycleOwner) {
                var bus = buses[lifecycleOwner]
                if (bus == null) {
                    bus = EventBusFactory(lifecycleOwner)
                    buses[lifecycleOwner] = bus
                    // TODO `?.` was added for unit test, we need to add lifecycle in the tests so better mock the
                    // LifecycleOwner
     ...
     */
    fun <T : ElfoEvent> emit(clazz: Class<T>, event: T) {
     ...
     */
    fun <T : ElfoEvent> getSafeManagedObservable(clazz: Class<T>): Observable<T> {
 ...
 */
inline fun <reified T : ElfoEvent> LifecycleOwner.emit(event: T) =
 ...
 */
inline fun <reified T : ElfoEvent> LifecycleOwner.getSafeManagedObservable(): Observable<T> =
